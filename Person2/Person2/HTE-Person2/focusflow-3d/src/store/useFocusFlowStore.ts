/**
 * FocusFlow — Zustand store (Person 6: enhanced with full PRD §5.1.1 learner model)
 * Client-side state management matching Supabase PostgreSQL schema.
 */
import { create } from 'zustand';
import type { KnowledgeGraph, LearnerStateSnapshot, ExplanationMode } from '@/rag/types';
import type {
  InteractionEvent,
  CognitiveState,
  SessionParams,
...
interface FocusFlowState {
  ...
  // Active panel state (which panel is open above the game canvas)
  activePanel: string | null;
  setActivePanel: (panel: string | null) => void;

  // Selected concept (from whiteboard panel)
...
export const useFocusFlowStore = create<FocusFlowState>((set, get) => ({
  ...
  updateLearnerConcept: (conceptId, mastery, lastSeen, errorPatterns = []) =>
    set((state) => {
      ...
      return { learnerState: { ...state.learnerState, concepts } };
    }),
  setCognitiveState: (cognitive_state) =>
    set((state) => ({
      learnerState: { ...state.learnerState, cognitive_state },
    })),
  ...
  // ── Event Queue ───────────────────────────────────────
  pendingEvents: [],
  pushEvent: (event) => set((state) => ({ pendingEvents: [...state.pendingEvents, event] })),
