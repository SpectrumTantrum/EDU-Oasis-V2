...
from app.rag.rag import upsert_material, make_source_id
...
def search_topic(topic: str, max_results: int = 5, skip_cache: bool = False) -> List[dict]:
    ...
    try:
        ...
        for i, r in enumerate(results.results):
            ...
            raw_text = getattr(r, "text", None) or getattr(r, "content", None) or ""
            if isinstance(raw_text, list):
                raw_text = " ".join(str(x) for x in raw_text)
            text = _safe_str(str(raw_text)[:500])
            out.append({
            ...
            if supabase and raw_text:
                try:
                    upsert_material(
                        supabase,
                        source_id=make_source_id("exa", url or f"{topic}-{i}"),
                        text=str(raw_text),
                        topic=topic,
