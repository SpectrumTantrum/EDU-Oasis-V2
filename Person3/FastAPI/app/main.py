...
from app.rag.database import Base ,engine, get_db
from app.rag import models  # ensures Post is registered
import os
try:
...
from app.rag.rag_ingest import ingest_text
from app.rag.rag_retrieve import search_chunks, format_chunks_as_resources
# Use Exa when EXA_API_KEY is set, else DuckDuckGo
_SEARCH_BACKEND = "duckduckgo"
SKIP_LEGACY_DB = os.environ.get("SKIP_LEGACY_DB") == "1"
try:
    if os.environ.get("EXA_API_KEY"):
        from app.rag.search_exa import search_topic, search_youtube, get_bookshelf_resources, get_cache_stats
        _SEARCH_BACKEND = "exa"
    else:
        from app.rag.search import search_topic, search_youtube, get_bookshelf_resources, get_cache_stats
except ImportError:
    from app.rag.search import search_topic, search_youtube, get_bookshelf_resources, get_cache_stats
...
# ────────────────────────────────────────────────────────────
# Person 3: RAG endpoints  (POST /rag/ingest  GET /rag/search)
# ────────────────────────────────────────────────────────────
...
@app.post("/rag/ingest", status_code=status.HTTP_200_OK)
def rag_ingest(body: RagIngestRequest):
    """
    Chunk *text*, generate OpenAI embeddings, and upsert into Supabase material_chunks.
    Requires SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, and OPENAI_API_KEY env vars.
    """
    client = get_supabase()
    if client is None:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Supabase not configured (missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY)",
        )
    ...
    if result.get("skipped") and result.get("reason") and "OPENAI_API_KEY" in (result.get("reason") or ""):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
...
def rag_search(
    ...
    topic: Optional[str] = None,
    as_resources: bool = True,
):
    """
    Embed *query*, run Supabase match_material_chunks RPC, return top-K results.
    Pass as_resources=false to get raw chunk rows instead of resource-shaped dicts.
    """
    client = get_supabase()
    if client is None:
        raise HTTPException(
    ...
    if as_resources:
        resources = format_chunks_as_resources(chunks, default_topic=topic or query)
        return {
            "query": query,
            "resources": resources,
            "source": "rag",
